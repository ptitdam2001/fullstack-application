...
- **Auth**: JWT stored in localStorage under key `'user'` (`{ user?, token? }`), read by `useCheckAuthLocalStorage`. `CheckAuthentication` guard wraps protected routes. Non-hook utilities in `src/Auth/authStorage.ts` (`getAuthToken`, `clearAuthStorage`) and `src/Auth/authNavigation.ts` (`redirectToLogin`) are usable outside React (e.g. axios interceptors). A response interceptor on `AXIOS_INSTANCE` in `config/axios-instance.ts` handles 401s: clears auth storage + redirects to login.
- **Mocking**: MSW (Mock Service Worker) is enabled in non-production builds. Mock handlers are auto-generated by orval alongside the SDK. To disable mocks and hit the real backend: `VITE_MOCKED_BACKEND=false pnpm --filter application-material dev`.
...
> **Gotcha — port**: `config/axios-instance.ts` hardcodes `http://localhost:4000/` as the backend URL. If you change `PORT` in the backend `.env`, update the axios instance accordingly (or expose it as `VITE_BACKEND_BASEURL`).
