...
type SeedUserRecord = Record<
  (typeof SEED_USERS)[number]["email"],
...
async function cleanup() {
  ...
  await prisma.championship.deleteMany({
    where: { name: SEED_CHAMPIONSHIP_NAME },
  });
...
async function seedRelations(users: SeedUserRecord, teamId: string) {
  ...
  await prisma.userTeam.create({
    data: {
      userId: users["coach@seed.local"].id,
      teamId,
      role: TeamRole.COACH,
    },
  });
  ...
  await prisma.userTeam.create({
    data: {
      userId: users["player@seed.local"].id,
      teamId,
  ...
  console.log(
    `  ✓ player@seed.local → PLAYER de "${SEED_TEAM_NAME}" (jersey #10)`,
  );
}
