...
- **Logs** : aucune donnée personnelle dans les logs applicatifs (adresse email, adresse IP, user-agent…), et aucun token hors développement. Le garde est **fail-closed** : `NoopEmailService` ne journalise le token que si `NODE_ENV` vaut explicitement `development` ou `test` (jamais s'il est absent ou différent), et jamais le destinataire ; sinon il journalise seulement qu'aucun email n'est envoyé. Les logs de requêtes HTTP se limitent à la méthode, au chemin (query string retirée, identifiants d'objets masqués), au statut, à la taille et à la durée — ni IP, ni user-agent, ni referer. Les erreurs journalisées se limitent au nom, au code (Prisma), au modèle et aux noms de champs concernés, plus la pile d'appels — **jamais le message** : les erreurs Prisma y embarquent les données de la requête (email, hash de mot de passe…). Ce nettoyage est appliqué une seule fois, dans le logger. Conséquence en production : les emails d'activation et de reset ne sont pas délivrés — activation par l'Admin uniquement, reset de mot de passe indisponible — jusqu'à l'arrivée d'un vrai `IEmailService`. L'image Docker backend fixe `NODE_ENV=production` et le script `start:dev` fixe `NODE_ENV=development`.
